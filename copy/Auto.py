...
async def TranslateAndExecute(commands: List[str]):
    ...
    for command in commands:
        if command.startswith("message "):
            ...
            if len(parts) == 2:
                funcs.append(send_whatsapp_message(parts[0], parts[1]))
        elif command.startswith("voice call "):
            funcs.append(whatsapp_voice_call(command.removeprefix("voice call ")))
        elif command.startswith("video call "):
            funcs.append(whatsapp_video_call(command.removeprefix("video call ")))
