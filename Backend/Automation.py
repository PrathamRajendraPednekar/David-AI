...
def log_call(name, call_type):
    """Log WhatsApp calls/messages with timestamp"""
    log_path = r"C:\Users\prath\OneDrive\Desktop\Jarvis\Data\CallHistory.json"
...
#send_whatsapp_message("ayush mangela", "How are you?")
...
VOICE_CALL_ICON_PATH = r"C:\Users\prath\OneDrive\Desktop\Jarvis\Backend\voice_call_icon.png"
...
def whatsapp_voice_call(name):
    ...
    if key in CONTACTS:
        ...
        if not os.path.exists(VOICE_CALL_ICON_PATH):
            ...
            TextToSpeech("Voice call icon not found.")
            return
        ...
        if call_button:
            ...
            TextToSpeech(f"Calling {name} on WhatsApp.")
            log_call(name, "voice")
        else:
            print(" Voice call button not detected.")
            TextToSpeech("Voice call button not detected.")
    else:
        print(" Contact not found.")
        TextToSpeech("Sorry, contact not found.")
...
VIDEO_CALL_ICON_PATH = r"C:\Users\prath\OneDrive\Desktop\Jarvis\Backend\video_call_icon.png"
...
def whatsapp_video_call(name):
    ...
    if key in CONTACTS:
        ...
        if not os.path.exists(VIDEO_CALL_ICON_PATH):
            ...
            TextToSpeech("Video call icon not found.")
            return
        ...
        if video_button:
            ...
            TextToSpeech(f"Video calling {name} on WhatsApp.")
            log_call(name, "video")
        else:
            print(" Video call button not detected.")
            TextToSpeech("Video call button not detected.")
    else:
        print(" Contact not found.")
        TextToSpeech("Sorry, contact not found.")
...
async def TranslateAndExecute(commands: List[str]):
    ...
    for command in commands:
        if command.startswith("message "):
            ...
            for contact in CONTACTS:
                if rest.startswith(contact):
                    ...
                    if message:
                        print(f"[DEBUG] Name: {name}, Message: {message}")  
                        funcs.append(asyncio.to_thread(send_whatsapp_message, name, message))
                    else:
                        print(" Message is empty.")
                        TextToSpeech("What should I send?")
                    break
            else:
        ...
        elif command.startswith("youtube play "):
            funcs.append(asyncio.to_thread(PlayYoutube, command.removeprefix("youtube play ")))
...
if __name__ == "__main__":
    if __name__ == "__main__":
        asyncio.run(Automation([
            ...
            #"duckduckgo search Future of AI"        #not working
            #"play AI revolution on youtube",        #not working  
            #"open visual studio code"
            #"open spotify" 
            #"content AI impact on jobs",            #not working
            #"system mute"
            #"message ayush mangela Hello,This is jarvis ",
